...
def search_user_by_username(user_name):
    try:
        r = requests.get('https://discoveryprovider3.audius.co/v1/users/search', params={
        'query': user_name
        }, headers=headers)
...
def search_user_favorites(user_id):
    try:
        r = requests.get(f'https://discoveryprovider3.audius.co/v1/users/{user_id}/favorites', headers=headers)
        results = r.json()['data']
        return results
    except Exception:
...
# MAIN FUNCTIONS
def get_all_user_data_by_user_name(user_name):
    try:
        r = search_user_by_username(user_name)
        artist_id = r['id']
        following_count = r['followee_count']
...
def get_user_follower_count_by_username(user_name):
    try:
        r = search_user_by_username(user_name)
        result = r['follower_count']
        return result
    except Exception:
        error_data = f'user follower count for {user_name}'
        handle_resolve_data_error(error_data, 3)

def get_user_following_count_by_username(user_name):
    try:
        r = search_user_by_username(user_name)
        result = r['followee_count']
        return result
    except Exception:
        error_data = f'user following count for {user_name}'
        handle_resolve_data_error(error_data, 3)

def get_user_track_count_by_username(user_name):
    try:
        r = search_user_by_username(user_name)
        result = r['track_count']
        return result
    except Exception:
        error_data = f'user track count for {user_name}'
        handle_resolve_data_error(error_data, 3)

def get_user_album_count_by_username(user_name):
    try:
        r = search_user_by_username(user_name)
        result = r['album_count']
        return result
    except Exception:
        error_data = f'user album count for {user_name}'
        handle_resolve_data_error(error_data, 3)

def get_user_favorites_count_by_username(user_name):
    try:
