...
def get_artist_track_list(artist_id):
    ...
    for track in artist_track_list:
        ...
        track_url = track[9]
        track_data = (track_id, artist_id, track_name, track_duration, track_url)
...
def find_track(browser, track_title):
    global found_track
    track_elements = browser.find_elements_by_class_name('TrackTile_title__1zakm')
...
def scroll(browser):
    ActionChains(browser).key_down(Keys.LEFT_ALT).key_down(Keys.DOWN).key_up(Keys.LEFT_ALT).key_up(Keys.DOWN).perform()
